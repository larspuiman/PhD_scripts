...
//    double conc_act[num_compounds];               /* Actual intracellular conc */
    ...
    for(int j = 0; j < num_timesteps; ++j) 
    {
        ...
        rk4vec ( current_time, conc_act, lagrangian_timestep, conc_ref, rates_IC_ref, elasticity_matrix, conc_new);
           /* calculate the derivatives one more time for checking the results. The dcdt's are being used to check 
            ...
            double dc_dt_act[num_compounds]; 
            conc_derivatives(conc_act, conc_ref, rates_IC_ref, elasticity_matrix, dc_dt_act);     /* [mol/m3/h] */
            ...
            if (nan_flag_count >= 1)
            {
                for( int i = 0; i < num_compounds; ++i)
                {
                    conc_act[i] = conc_store[i][store_index-1]; 
                }
            }
    ...
    sprintf(Filename_conc, "conc_data_batch.dat");
    sprintf(Filename_dcdt, "dcdt_data_batch.dat");
    sprintf(Filename_time, "t_data_batch.dat");

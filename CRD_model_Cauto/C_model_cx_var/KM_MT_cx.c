...
void rk4vec ( double t0, double u0[12], double dt, double conc_ref[12], double rates_IC_ref[11], double elasticity_matrix[11][12], double *u, double CX )
...
*/
  conc_derivatives ( u0 , conc_ref, rates_IC_ref, elasticity_matrix, f0, CX ) ;
  ...
  conc_derivatives (  u1,  conc_ref, rates_IC_ref, elasticity_matrix, f1, CX );
  ...
  conc_derivatives ( u2,  conc_ref, rates_IC_ref, elasticity_matrix, f2, CX );
  ...
  conc_derivatives ( u3, conc_ref, rates_IC_ref, elasticity_matrix, f3, CX );
/*
...
void conc_derivatives(double conc_act[12], double conc_ref[12], double rates_IC_ref[11], double elasticity_matrix[11][12], double *dc_dt, double CX)
{    
    ...
    double kla_co = 0.0498; double cs_co = 1.0865;
    double kla_h2 = 0.0742; double cs_h2 = 0.3959;
    double kla_co2 = 0.0484; double cs_co2 = 20.2281;

    double MTC_co = kla_co*(cs_co - conc_eff[0]) * 3600;        /* mol/m3/h*/
    double MTC_h2 = kla_h2*(cs_h2 - conc_eff[1]) * 3600; 
    double MTC_co2 = kla_co*(cs_co2 - conc_eff[2]) * 3600; 

    dc_dt[0] = MTC_co + dc_dt[0];
    dc_dt[1] = MTC_h2 + dc_dt[1];
    dc_dt[2] = MTC_co2 + dc_dt[2];
...
//    double conc_act[num_compounds];               /* Actual intracellular conc */
    ...
    for (double CX = 200; CX <= 200; CX += 50) 
    {
        ...
        conc_derivatives(conc_ini, conc_ref, rates_IC_ref, elasticity_matrix, dc_dt_new, CX );
        ...
        printf("conc X = %f, conc CO = %f, conc H2 = %f, conc Ac = %f\n", CX, conc_act[0], conc_act[1], conc_act[3]);
        ...
        for(int j = 0; j < num_timesteps; ++j) 
        {
            ...
            rk4vec ( current_time, conc_act, lagrangian_timestep, conc_ref, rates_IC_ref, elasticity_matrix, conc_new, CX);
            ...
            conc_derivatives(conc_act, conc_ref, rates_IC_ref, elasticity_matrix, dc_dt_new, CX);
            for(int i = 0; i < num_compounds; ++i)
        ...
        sprintf(Filename_conc, "Results_cx_var/conc_data_CX_%g.dat", (double) CX);
        sprintf(Filename_dcdt, "Results_cx_var/dcdt_data_CX_%g.dat", (double) CX);
        sprintf(Filename_time, "Results_cx_var/t_data_CX_%g.dat", (double) CX);

...
//    double conc_act[num_compounds];               /* Actual intracellular conc */
    ...
    double CO_av = 0.0947;
    double H2_av = 0.1940;
    double CO_std = 0.0491;
    double H2_std = 0.0575;
    double CO2_av = 20;
    ...
    for (double factor = 2.025; factor <= 2.075; factor += 0.025) 
    {
        double freq = 1.0;
        ...
        printf("conc freq = %g s, factor amplitude = %g \n", (double) freq, (double) factor);
        ...
        for(int j = 0; j < num_timesteps; ++j) 
        {
            ...
            conc_act[0] = CO_std * factor * sin( (2.0*3.1415 / freq) * (current_time*3600)) + CO_av;
            conc_act[1] = H2_std * factor * sin( (2.0*3.1415 / freq) * (current_time*3600)) + H2_av;
            conc_act[2] = CO2_av;
        ...
        sprintf(Filename_conc, "Results_C_script_amp/conc_data_amp_%g.dat", (double) factor);
        sprintf(Filename_dcdt, "Results_C_script_amp/dcdt_data_amp_%g.dat", (double) factor);
        sprintf(Filename_time, "Results_C_script_amp/t_data_amp_%g.dat", (double) factor);

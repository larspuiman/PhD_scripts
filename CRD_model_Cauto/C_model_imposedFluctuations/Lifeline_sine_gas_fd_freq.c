...
{    
    ...
    double CX = 150;                                 /* biomass concentration LATER [molx/m3] */
...
//    double conc_act[num_compounds];               /* Actual intracellular conc */
    ...
    double CO_av = 0.0947;
    double H2_av = 0.1940;
    double CO_std = 0.0491*2.0;
    double H2_std = 0.0575*2.0;
    double Fd_std = 0.6652*2.0;
    double Fd_av = 5.3336;
    double CO2_av = 20;
    ...
    for (double freq = 0; freq <= 1; freq += 100) 
    {
        ...
        for(int j = 0; j < num_timesteps; ++j) 
        {
            ...
            /* set CO, H2, CO2 as initial conc from sine function */
            double t_proxy = (double) euler_counter/timestep_diff; 
            if (freq > 0) {
                conc_act[0] = CO_std * sin( (2.0*3.1415 / freq) * (current_time*3600)) + CO_av;
                conc_act[1] = H2_std * sin( (2.0*3.1415 / freq) * (current_time*3600)) + H2_av;
                conc_act[2] = CO2_av;
                conc_act[11] = Fd_std * sin( (2.0*3.1415 / freq) * (current_time*3600)) + Fd_av;
                }
                else {
                conc_act[0] = CO_av;
                conc_act[1] = H2_av;
        ...
        sprintf(Filename_conc, "Results_C_script_gas_fd/conc_data_freq_%g_s.dat", (double) freq);
        sprintf(Filename_dcdt, "Results_C_script_gas_fd/dcdt_data_freq_%g_s.dat", (double) freq);
        sprintf(Filename_time, "Results_C_script_gas_fd/t_data_freq_%g_s.dat", (double) freq);
